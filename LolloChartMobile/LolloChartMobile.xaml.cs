...
namespace LolloChartMobile
{
    public sealed partial class LolloChart : UserControl // where T : class, new()
    {
        ...
        private Boolean IsHandlersActive = false;
        private void AddHandlers()
        {
            if (IsHandlersActive == false)
            {
                ...
                // (this.Parent as Panel).SizeChanged +=new SizeChangedEventHandler(LolloChart_SizeChanged);
        ...
        {
            // this.IsVisibleChanged -= LolloChart_IsVisibleChanged;
            _appView.VisibleBoundsChanged -= OnAppView_VisibleBoundsChanged;
            //_appView.Consolidated -= OnAppView_Consolidated;
            this.IsEnabledChanged -= OnIsEnabledChanged;
            this.SizeChanged -= OnSizeChanged;
            //(this.Parent as Panel).SizeChanged -= LolloChart_SizeChanged;
        ...
        private void SetFixedSizes()
        {
            ...
            MainRow0.MinHeight = MainRow0.MaxHeight = (double)this.Resources["GridHeightSmall"];
            MainRow1.MinHeight = MainRow1.MaxHeight = (double)this.Resources["GridHeightSmall"];
            MainRow2.MinHeight = (double)this.Resources["GridHeightLarge"];
            MainRow3.MinHeight = MainRow3.MaxHeight = (double)this.Resources["GridHeightSmall"];
            MainRow4.MinHeight = MainRow4.MaxHeight = (double)this.Resources["GridHeightSmall"];
            MainCol0.MinWidth = MainCol0.MaxWidth = (double)this.Resources["GridWidthSmall"];
            MainCol1.MinWidth = (double)this.Resources["GridWidthLarge"];
            MainCol2.MinWidth = MainCol2.MaxWidth = (double)this.Resources["GridWidthSmall"];
            this.MinHeight = MainRow0.MinHeight + MainRow1.MinHeight + MainRow2.MinHeight + MainRow3.MinHeight + MainRow4.MinHeight;
            this.MinWidth = MainCol0.MinWidth + MainCol1.MinWidth + MainCol2.MinWidth;
        }
        ...
        {
            if (this.Visibility == Windows.UI.Xaml.Visibility.Visible && e.OldValue != e.NewValue) SetLineStyles();
        }
        ...
        {
            if (this.Visibility == Windows.UI.Xaml.Visibility.Visible && _appView.Orientation != _prevOrientation)
            {
                _prevOrientation = _appView.Orientation;
                InvalidateMeasure();
                //SetVariableGridSize();
        ...
        {
            if (GridChartArea.Visibility == Windows.UI.Xaml.Visibility.Visible && GridChartArea.ActualHeight > 0.0 && GridChartArea.ActualWidth > 0.0)
            {
        ...
        {
            //must do the following otherwise the stupid thing keeps growing and growing forever if the owner is dumb
            ...
            else
            {
                ...
                Size beforeCentre = GridChartArea.DesiredSize; // just to see it in the debugger
                ...
                Size afterCentre = GridChartArea.DesiredSize;
                if (afterCentre.Height == 0.0 || afterCentre.Width == 0.0) return new Size(0.0, 0.0);
        ...
        //        double availableWidth = Windows.ApplicationModel.Core.CoreApplication.MainView.CoreWindow.Bounds.Width;
        //        double availableHeight = Windows.ApplicationModel.Core.CoreApplication.MainView.CoreWindow.Bounds.Height;

        //        double maxAcceptableWidth = Math.Min(Math.Min(imposedWidth, imposedMaxWidth), availableWidth);
        //        double maxAcceptableHeight = Math.Min(Math.Min(imposedHeight, imposedMaxHeight), availableHeight);

        //        double widthAvailableForCentre = maxAcceptableWidth - MainCol0.MinWidth - MainCol2.MinWidth; //- afterLeft.Width - afterRight.Width;
        //        double heightAvailableForCentre = maxAcceptableHeight - MainRow0.MinHeight - MainRow1.MinHeight - MainRow3.MinHeight - MainRow4.MinHeight;//- afterTop.Height - afterBottom.Height;  //widthForCentre * 2 / 3;
        //        //GridChartArea.Measure(new Size(widthAvailableForCentre, heightAvailableForCentre));
        //        //Size afterCentre = GridChartArea.DesiredSize;
        //        //if (afterCentre.Height == 0.0 || afterCentre.Width == 0.0) return new Size(0.0, 0.0);
        //        //else
        //        //{
        //        //    return new Size(widthAvailableForCentre, heightAvailableForCentre);
        //        //}
        ...
        public void Draw()
        {
            ...
            if (XGridScale != null && Y1GridScale != null && XY1DataSeries != null)
            {
                if (_XY1DataSeries_Internal == null)
                    _XY1DataSeries_Internal = new XYDataSeries_Internal(this.GridChartArea, XGridScale.ScaleType, Y1GridScale.ScaleType,
            ...
            if (XGridScale != null && Y2GridScale != null && XY2DataSeries != null)
            {
                if (_XY2DataSeries_Internal == null)
                    _XY2DataSeries_Internal = new XYDataSeries_Internal(this.GridChartArea, XGridScale.ScaleType, Y2GridScale.ScaleType,
                    XGridScale.Min, XGridScale.Max,
            ...
            if (XGridScale != null && Y1GridScale != null && XY3DataSeries != null)
            {
                if (_XY3DataSeries_Internal == null)
                    _XY3DataSeries_Internal = new XYDataSeries_Internal(this.GridChartArea, XGridScale.ScaleType, Y1GridScale.ScaleType,
                    XGridScale.Min, XGridScale.Max,
                    Y1GridScale.Min, Y1GridScale.Max,
            ...
            if (XGridScale != null && Y2GridScale != null && XY4DataSeries != null)
            {
                if (_XY4DataSeries_Internal == null)
                    _XY4DataSeries_Internal = new XYDataSeries_Internal(this.GridChartArea, XGridScale.ScaleType, Y2GridScale.ScaleType,
                    XGridScale.Min, XGridScale.Max,
                    Y2GridScale.Min, Y2GridScale.Max,
            ...
            {
                if (_XPrimaryGridLines_Internal == null) _XPrimaryGridLines_Internal = new XGridLines_Internal(this.GridChartArea, XGridScale.ScaleType, XPrimaryGridLines.Points, XGridScale.Min, XGridScale.Max, PrimaryGridLineThickness, ChartObjectTag.X1PrimaryGridlines, GridLineStyle);
            ...
            {
                if (_XSecondaryGridLines_Internal == null) _XSecondaryGridLines_Internal = new XGridLines_Internal(this.GridChartArea, XGridScale.ScaleType, XSecondaryGridLines.Points, XGridScale.Min, XGridScale.Max, SecondaryGridLineThickness, ChartObjectTag.X1SecondaryGridlines, GridLineStyle);
            ...
            {
                if (_YPrimaryGridLines_Internal == null) _YPrimaryGridLines_Internal = new YGridLines_Internal(this.GridChartArea, Y1GridScale.ScaleType, YPrimaryGridLines.Points, Y1GridScale.Min, Y1GridScale.Max, PrimaryGridLineThickness, ChartObjectTag.Y1PrimaryGridlines, GridLineStyle);
            ...
            {
                if (_YSecondaryGridLines_Internal == null) _YSecondaryGridLines_Internal = new YGridLines_Internal(this.GridChartArea, Y1GridScale.ScaleType, YSecondaryGridLines.Points, Y1GridScale.Min, Y1GridScale.Max, SecondaryGridLineThickness, ChartObjectTag.Y1SecondaryGridlines, GridLineStyle);
        ...
        {
            if (whichDataSeries == this.XY1DataSeries)
            {
                _XY1DataSeries_Internal.DrawCross(pointX, pointY);
            }
            else if (whichDataSeries == this.XY2DataSeries)
            {
                _XY2DataSeries_Internal.DrawCross(pointX, pointY);
            }
            else if (whichDataSeries == this.XY3DataSeries)
            {
                _XY3DataSeries_Internal.DrawCross(pointX, pointY);
            }
            else if (whichDataSeries == this.XY4DataSeries)
            {
    ...
    internal abstract class GridLines_Internal : ScalableChartObject
    {
        ...
        internal virtual void ReInit(ScaleType scaleType, double[] points, double minimum, double maximum)
        {
            ...
            if (scaleType != ScaleType || points != DataPoints || minimum != Minimum || maximum != Maximum)
            {
                ...
                for (int i = 0; i <= this.DataPoints.GetUpperBound(0); i++)
                {
    ...
    internal class XGridLines_Internal : GridLines_Internal
    {
        ...
        {
            if (Container.ActualHeight == 0.0 || Container.ActualWidth == 0.0) return;
            double[] drawPoints = new double[this.DataPoints.GetUpperBound(0) + 1];
            if (this.ScaleType == ScaleType.Linear)
            {
                drawPoints = Scaler.ScaleLinear(Minimum, Maximum, 0, Container.ActualWidth, this.DataPoints);
            }
            else if (this.ScaleType == ScaleType.Logarithmic)
            {
                drawPoints = Scaler.ScaleLogarithmic(Minimum, Maximum, 0, Container.ActualWidth, this.DataPoints);
            }
    ...
    internal class YGridLines_Internal : GridLines_Internal
    {
        ...
        protected override void Draw()
        {
            ...
            if (this.ScaleType == ScaleType.Linear)
            {
                drawPoints = Scaler.ScaleLinear(Minimum, Maximum, 0, Container.ActualHeight, this.DataPoints);
            }
            else if (this.ScaleType == ScaleType.Logarithmic)
            {
                drawPoints = Scaler.ScaleLogarithmic(Minimum, Maximum, 0, Container.ActualHeight, this.DataPoints);
            }
    ...
    internal class XGridLabels_Internal : GridLabels_Internal
    {
        ...
        protected override void Draw()
        {
            ...
            if (this.ScaleType == ScaleType.Linear)
            {
                drawPoints = Scaler.ScaleLinear(this.DataPoints[0], this.DataPoints[this.DataPoints.GetUpperBound(0)], 0, Container.ActualWidth, this.DataPoints);
            }
            else if (this.ScaleType == ScaleType.Logarithmic)
            {
                drawPoints = Scaler.ScaleLogarithmic(this.DataPoints[0], this.DataPoints[this.DataPoints.GetUpperBound(0)], 0, Container.ActualWidth, this.DataPoints);
            }
            for (int i = 0; i <= drawPoints.GetUpperBound(0); i++)
            {
                try
                {
                    // MyTBs[i].Text = Convert.ToDecimal(this.DataPoints[i]).ToString(DataPointsFormat, CultureInfo.CurrentUICulture);
                    MyTBs[i].Text = this.DataPoints[i].ToString(DataPointsFormat, CultureInfo.CurrentUICulture);
                }
                catch (FormatException)
                {
                    // MyTBs[i].Text = Convert.ToDecimal(this.DataPoints[i]).ToString(CultureInfo.CurrentUICulture);
                    MyTBs[i].Text = this.DataPoints[i].ToString(CultureInfo.CurrentUICulture);
                }
                Canvas.SetLeft(MyTBs[i], drawPoints[i] - (offset * MyTBs[i].Text.Length));
                Canvas.SetTop(MyTBs[i], 0.0);
                Canvas.SetZIndex(MyTBs[i], 999);
                //MyTBs[i].Content = Convert.ToDecimal(this.DataPoints[i]).ToString();
    ...
    internal class YGridLabels_Internal : GridLabels_Internal
    {
        ...
        protected override void Draw()
        {
            ...
            if (this.ScaleType == ScaleType.Linear)
            {
                drawPoints = Scaler.ScaleLinear(this.DataPoints[0], this.DataPoints[this.DataPoints.GetUpperBound(0)], 0, Container.ActualHeight, this.DataPoints);
            }
            else if (this.ScaleType == ScaleType.Logarithmic)
            {
                drawPoints = Scaler.ScaleLogarithmic(this.DataPoints[0], this.DataPoints[this.DataPoints.GetUpperBound(0)], 0, Container.ActualHeight, this.DataPoints);
            }
            for (int i = 0; i <= drawPoints.GetUpperBound(0); i++) //the Panel coordinates start from the top left, while the Y axis starts from the bottom left
            {
    ...
    internal class XYDataSeries_Internal : ChartObject
    {
            ...
            : base(container, tag)
        {
            ...
            switch (sda)
            {
                case StrokeDashArrays.Dashed: MyPolyline.StrokeDashArray = Dashed;
                    break;
                case StrokeDashArrays.Continue: MyPolyline.StrokeDashArray = null;
                    break;
                default:
                    break;
            }
        ...
        protected override void Draw()
        {
            if (this.DataPoints == null || Container.ActualHeight == 0.0 || Container.ActualWidth == 0.0) return;
        ...
        //            Convert.ToDouble(CrossPointX) / Convert.ToDouble(this.DataPoints.GetUpperBound(0) + 1) * Convert.ToDouble(this.MyPolyline.Points.Count)
        //            ));
        //        if (_isHistogram)
        //        {
        ...
        internal void DrawCross(int pointX, double pointY)
        {
            ...
            if (pointX >= 0 && pointX <= this.DataPoints.GetUpperBound(0))
            {
                CrossPointX = pointX;
                CrossPointY = pointY;
            }
